...
const ProjectCard: React.FC<ProjectCardProps> = ({ project }) => {
  return (
    <Link href={`/projects/${project.slug}`}>
      <div
        key={project.title}
        className=" flex flex-col xl:flex-row lg:flex-col  tablet:flex-row tablet:space-x-8 xl:space-x-8 lg:space-x-0 items-center p-4 rounded-xl transition-transform transform md:hover:scale-[102%]"
      >
        <div className="flex-shrink-0">
          ...
          <Image
            ...
            width={256}
            height={160}
            className="rounded-lg object-cover md:w-64 h-40 transition-transform duration-300 ease-in-out"
          />
        </div>
